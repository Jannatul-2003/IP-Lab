...
function ReadOnlyBanner() {
  return (
    <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-50 border border-amber-100 text-amber-700 text-sm mb-6">
      ...
      <span>Faculty Advisor — read-only view. No modifications permitted.</span>
    </div>
  );
}
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <div>
              ...
              <p className="text-gray-400 mt-1 text-sm">CSEDU Students&apos; Club — Term 8 Overview</p>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                { label: "Active Members", value: activeMembers.length, icon: "👥", color: "text-blue-600" },
                { label: "Pending Apps", value: pendingMembers.length, icon: "⏳", color: "text-orange-500" },
                { label: "Approved Budget", value: formatCurrency(totalBudget), icon: "💰", color: "text-green-600" },
                { label: "Total Spent", value: formatCurrency(totalSpent), icon: "📤", color: "text-red-500" },
              ].map((s) => (
            ...
            <div className="card">
              <h2 className="font-heading text-lg font-semibold text-primary mb-4 flex items-center gap-2">
                <DollarSign className="w-5 h-5 text-green-500" /> Finance Summary
              </h2>
                    ...
                    <div key={b.id} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100 text-sm">
                      <div>
                        <p className="font-medium text-primary">
                          {b.eventId ? "Event Budget" : "General Term Budget"}
                        </p>
                        <p className="text-xs text-gray-400">Spent: {formatCurrency(spent)} / {formatCurrency(b.totalAmountBdt)}</p>
                      </div>
            ...
            <div className="card">
              <h2 className="font-heading text-lg font-semibold text-primary mb-4 flex items-center gap-2">
                <Vote className="w-5 h-5 text-purple-500" /> Current Election
              </h2>
              <div className="flex items-center justify-between p-4 bg-purple-50 rounded-xl border border-purple-100">
                <div>
                  <p className="font-semibold text-primary">EC Election 2026 — Term 8</p>
                  <p className="text-sm text-gray-400 mt-0.5">
                    Phase 1: {formatDate(mockElection.phase1Start)} – {formatDate(mockElection.phase1End)}
                  </p>
                  <p className="text-sm text-gray-400">
                    Phase 2: {mockElection.phase2Start ? `${formatDate(mockElection.phase2Start)} – ${formatDate(mockElection.phase2End!)}` : "Pending"}
                  </p>
                </div>
            ...
            <div className="card">
              <h2 className="font-heading text-lg font-semibold text-primary mb-4 flex items-center gap-2">
                <FileText className="w-5 h-5 text-orange-400" /> Recent Notices
              </h2>
                ...
                {mockNotices.slice(0, 4).map((n) => (
                  <div key={n.id} className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-300 mt-1">by {n.authorRole} · {formatDate(n.publishedAt)}</p>
                    </div>
                  </div>
                ))}
              </div>

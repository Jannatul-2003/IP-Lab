  ...
  return (
    <PageLayout>
      <div className="pt-20 pb-16 bg-slate-50 min-h-screen">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="card">
            ...
            {event.description && (
              <div className="mb-6">
                <h2 className="font-heading text-lg font-semibold text-primary mb-3">{t("events.aboutEvent")}</h2>
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-slate-100">
              ...
              {!user && event.status === "PUBLISHED" && (
                <p className="text-sm text-gray-400">
                  <a href="/login" className="text-accent hover:underline">{t("auth.signIn")}</a> {t("events.toRsvp")}
                </p>
              )}
            </div>
          </motion.div>
      ...
      <ConfirmDialog
        ...
        confirmLabel={t("events.cancelConfirmButton")}
        isLoading={loading}
      />
    </PageLayout>
  );
